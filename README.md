...
V1.6 系列是 Markdown-only cross-platform prompt adapter 和 README polish release。它把现有 Tutor Skill 打包成 full Skill、custom bot、Lite Prompt 和 API prompt 用法，并让根 README 更清楚展示 Core Reasoning Flow、平台入口和质量评估入口；不新增 source packs、网站、脚本、API 集成代码、包管理、数据库、持久记忆、真实 RAG / vector database、PDF、课程地图或基础设施。
