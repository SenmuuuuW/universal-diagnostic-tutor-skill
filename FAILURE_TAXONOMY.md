...
| Entrypoint duplication bloat | A thin sub-skill copies too much main Tutor logic. | `tutor-learn-path/SKILL.md` becomes another full `universal-diagnostic-tutor/SKILL.md`. | tutor-* SKILL files, AGENTS | Normal |
| Prompt shortcut / skill entrypoint confusion | Docs imply ordinary chat platforms expose Codex Skill menu commands. | A Lite Prompt user expects `tutor-learn-path` to appear as a button in normal chat. | COMMAND_SURFACE, PORTABILITY, USER_GUIDE | Normal |
