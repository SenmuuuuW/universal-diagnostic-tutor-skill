...
| Natural intent not recognized | The single tutor fails to route a natural-language request to the right internal behavior. | "我想系统学机器学习" gets a generic answer instead of goal clarification and a compact plan. | SKILL.md description triggers, intent routing, examples, evals | Normal |
| Obsolete (entrypoint duplication bloat) | Removed with the 2.0 single-tutor surface: thin sub-skill folders no longer exist. | Retained for historical failure records only. | — | Normal |
| Obsolete (prompt shortcut / entrypoint confusion) | Removed with the 2.0 single-tutor surface: no feature menu or command surface is advertised. | Retained for historical failure records only. | — | Normal |
