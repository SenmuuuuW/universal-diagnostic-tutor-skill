...
V1.2.2 是小型优化 release：强化 STEM / 理科 / AI-CS 优先定位、技术题领域诊断、零基础停顿节奏，以及 `\(...\)` / `\[...\]` 数学显示规则。本 Skill 仍保留跨学科诊断式教学能力，不新增 source packs、脚本、网站、API、数据库、持久记忆或基础设施。

## V1.3 说明

V1.3 强化真实老师感：普通教学回答不暴露内部 Skill / protocol 信息；更自然地做知识系统定位；用直觉和应用桥梁解释抽象 STEM 概念；并在合适时提炼可迁移的题型线索。它仍是 Markdown-only 教学行为更新，不新增 source packs、脚本、网站、API、数据库、持久记忆或基础设施。

## V1.4 说明

V1.4 增加 Learning Efficiency Optimization Loop：Tutor 会优先选择 next best teaching step，管理认知负荷，把学习者回应解释成 mastery signal，并避免重复讲学习者已经会的前置知识。它仍是 Markdown-only 教学行为更新，不新增 source packs、脚本、网站、API、数据库、持久记忆或基础设施。
